...
def entropy_regularized_opt_transport_dual_dist_torch(
    r: torch.Tensor,
    c: torch.Tensor,
    m: torch.Tensor,
    lmbda: float,
    ...
    r_non_zero_indices = torch.argwhere(r > 0).squeeze()
    c_non_zero_indices = torch.argwhere(c > 0).squeeze()

    select_m = m[torch.meshgrid(r_non_zero_indices, c_non_zero_indices)]
    k_mat = torch.exp(-select_m * lmbda)
    p_mat = sinkhorn_algorithm_torch(
    ...
    return torch.sum(p_mat * select_m)
